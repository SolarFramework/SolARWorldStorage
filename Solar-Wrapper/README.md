...
Our wrapper uses the `ETSIWorldStorage` [XPCF](https://github.com/b-com-software-basis/xpcf) component of our [Solar world storage module](https://github.com/SolarFramework/SolARModuleWorldStorage/tree/develop) as an implementation for SolAR framework `IWorldGraphManager`.
This component is a singleton, and instantiated as follows:

![XPCF component manager instantiation](images/XPCFComponentManager.png)

It is responsible for storing and managing `StorageElements` (`Trackables` and `Anchors`) as well as `Links`.
...
Each datastructure is described in detail in the [SolAR Framework git repository](https://github.com/SolarFramework/SolARFramework/tree/master/interfaces/datastructure).

**World storage module**
...
- **m_tags** : `multimap<pair<string, string>, shared_ptr<StorageWorldElement>>`
  - A multimap that allows for a tag-based lookup of `StorageWorldElement`s, a tag being key-value pair of strings.
